...
<!--
    # Author: Adrian Nowak
    # Notes: PHP script to make html table with microformats
	# Resources:
	# schema.org/
	...
	while($row = mysqli_fetch_array($DBdata)){
		if($id != $row["idChampions"] ){ //if it's new player
			...
			//Player Name and Wiki Link
				echo "<td><div itemscope itemtype='http://schema.org/Person'><a itemprop='additionalType' href='" . $row["playerLink"] . "'><span itemprop='name'>" . $row["playerName"] . "</span></a></div></td>";
				
			//Years
				echo "<td><div itemscope itemtype='http://schema.org/Event'>"; 
				echo "<span itemprop='description'>" . $row["firstYear"] . "</span>"; //there always will be a first year
				
				//Check for second year:
					$yearTwo = $row["secondYear"];
					if ($yearTwo != ''){ echo "<br /><span itemprop='description'>" .$yearTwo . "</span>";}
				
				//Check for third year:
					$yearThree = $row["thirdYear"];
					if ($yearThree != ''){ echo "<br /><span itemprop='description'>" .$yearThree . "</span>";}
			...
			//Countries:
				//add first country
				echo "<td><div itemscope itemtype='http://schema.org/Thing'><img itemprop='image' src='" 
				. $row["countryFlag"] . "' alt='Country Image' /><a itemprop='additionalType' target='_blank' href='" 
				. $row["countryLink"] . "'>" . "<span itemprop='address' itemprop='name'>" 
